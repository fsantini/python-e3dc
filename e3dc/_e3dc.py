...
class E3DC:
    ...
    def poll_rscp(self, keepAlive = False):
        ...
        """        
        if self.lastRequest is not None and (time.time() - self.lastRequestTime) < REQUEST_INTERVAL_SEC_LOCAL:
        ...
        sc = round(self.sendRequest( ('EMS_REQ_SELF_CONSUMPTION', 'None', None), keepAlive=True )[2],2)
    ...
    def poll_switches(self, keepAlive = False):
        ...
        switchDesc = self.sendRequest( ("HA_REQ_DATAPOINT_LIST", "None", None), keepAlive=True )
        switchStatus = self.sendRequest( ("HA_REQ_ACTUATOR_STATES", "None", None) , keepAlive=keepAlive)
    ...
    def set_switch_onoff(self, switchID, value, keepAlive = False):
        ...
        result = self.sendRequest( ("HA_REQ_COMMAND_ACTUATOR", "Container", [
                    ("HA_DATAPOINT_INDEX", "Uint16", switchID),
                    ("HA_REQ_COMMAND", "CString", cmd)]), keepAlive=keepAlive)
    ...
    def get_idle_periods(self, keepAlive = False):
        ...
        """    
        idlePeriodsRaw = self.sendRequest(('EMS_REQ_GET_IDLE_PERIODS', 'None', None), keepAlive = keepAlive)
        ...
        for period in idlePeriodsRaw[2]:
            ...
            periodObj = { 'day': day, 'start': [startHour, startMin], 'end': [endHour, endMin], 'active': active }
    ...
    def set_idle_periods(self, idlePeriods, keepAlive = False):
                            ...
                            if idlePeriod.keys() & ['active', 'start', 'end' ]:
                                if 'active' in idlePeriod:
                                    if isinstance(idlePeriod['active'], bool):
                                        ...
                                        raise TypeError('period ' + str(idlePeriod['day']) + ' in ' + idle_type + ' not a bool')
                                    
                                for key in ['start','end']:
                                    if key in idlePeriod:
                                        if isinstance(idlePeriod[key], list) and len(idlePeriod[key]) == 2:                                        
                                            for i in range(2):
                                                if isinstance(idlePeriod[key][i], int):
                                                    if idlePeriod[key][i] >= 0 and ((i == 0 and idlePeriod[key][i] < 24) or (i == 1 and idlePeriod[key][i] < 60)):
                                                        idlePeriod[key][i] = idlePeriod[key][i]
                                                    else:
                                                        raise ValueError(key in ' period ' + str(idlePeriod['day']) + ' in ' + idle_type + ' is not between 00:00 and 23:59')
                                if (idlePeriod['start'][0]*60 + idlePeriod['start'][1]) < (idlePeriod['end'][0]*60 + idlePeriod['end'][1]):
                                    periodList.append(('EMS_IDLE_PERIOD', 'Container', [
                                                        ('EMS_IDLE_PERIOD_TYPE', 'UChar8', self.IDLE_TYPE[idle_type]),
                                                        ('EMS_IDLE_PERIOD_DAY', 'UChar8', idlePeriod['day']),
                                                        ('EMS_IDLE_PERIOD_ACTIVE', 'Bool', idlePeriod['active'] ),
                                                        ('EMS_IDLE_PERIOD_START', 'Container', [
                                                            ('EMS_IDLE_PERIOD_HOUR', 'UChar8', idlePeriod['start'][0]),
                                                            ('EMS_IDLE_PERIOD_MINUTE', 'UChar8', idlePeriod['start'][1])]),
                                                        ('EMS_IDLE_PERIOD_END', 'Container', [
                                                            ('EMS_IDLE_PERIOD_HOUR', 'UChar8', idlePeriod['end'][0]),
                                                            ('EMS_IDLE_PERIOD_MINUTE', 'UChar8', idlePeriod['end'][1])])]) )
                                else:
                                    raise ValueError('end time is smaller than start time in period ' + str(idlePeriod['day']) + ' in ' + idle_type + ' is not between 00:00 and 23:59')
        ...
        result = self.sendRequest( ('EMS_REQ_SET_IDLE_PERIODS', 'Container', periodList), keepAlive = keepAlive)
    ...
    def get_pvi_data(self, keepAlive = False):
        ...
        acApparentPower = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_APPARENTPOWER'), 'PVI_VALUE')[2],2)
        acCurrent = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_CURRENT'), 'PVI_VALUE')[2],2)
        acEnergyAll = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_ENERGY_ALL'), 'PVI_VALUE')[2],2)
        acPower = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_POWER'), 'PVI_VALUE')[2],2)
        acReactivePower = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_REACTIVEPOWER'), 'PVI_VALUE')[2],2)
        acVoltage = round(rscpFindTag(rscpFindTag(req, 'PVI_AC_VOLTAGE'), 'PVI_VALUE')[2],2)
        dcCurrent = round(rscpFindTag(rscpFindTag(req, 'PVI_DC_CURRENT'), 'PVI_VALUE')[2],2)
        dcPower = round(rscpFindTag(rscpFindTag(req, 'PVI_DC_POWER'), 'PVI_VALUE')[2],2)
        dcVoltage = round(rscpFindTag(rscpFindTag(req, 'PVI_DC_VOLTAGE'), 'PVI_VALUE')[2],2)
        ...
        temperature = round(rscpFindTag(rscpFindTag(req, 'PVI_TEMPERATURE'), 'PVI_VALUE')[2],2)
    ...
    def get_power_settings(self, keepAlive = False):
        ...
        res = self.sendRequest(("EMS_REQ_GET_POWER_SETTINGS", "None", None), keepAlive = keepAlive)
        ...
        weatherRegulatedChargeEnabled = rscpFindTag(res, 'EMS_WEATHER_REGULATED_CHARGE_ENABLED')[2]
    ...
    def set_power_limits(self, enable, max_charge = None, max_discharge = None, discharge_start = None, keepAlive = False):
        ...
        if enable:
            ...
            res = self.sendRequest(("EMS_REQ_SET_POWER_SETTINGS", "Container", [ ("EMS_POWER_LIMITS_USED", "Bool", False) ]), keepAlive = keepAlive)
    ...
    def set_powersave(self, enable, keepAlive = False):
        ...
        """
        if enable:
            res = self.sendRequest(("EMS_REQ_SET_POWER_SETTINGS", "Container", [ ('EMS_POWERSAVE_ENABLED', 'UChar8', 1) ]), keepAlive = keepAlive)
        else:
            res = self.sendRequest(("EMS_REQ_SET_POWER_SETTINGS", "Container", [ ("EMS_POWERSAVE_ENABLED", "UChar8", 0) ]), keepAlive = keepAlive)
    ...
    def set_weather_regulated_charge(self, enable, keepAlive = False):
        ...
        """
        if enable:
            res = self.sendRequest(("EMS_REQ_SET_POWER_SETTINGS", "Container", [ ('EMS_WEATHER_REGULATED_CHARGE_ENABLED', 'UChar8', 1) ]), keepAlive = keepAlive)
        else:
            res = self.sendRequest(("EMS_REQ_SET_POWER_SETTINGS", "Container", [ ("EMS_WEATHER_REGULATED_CHARGE_ENABLED", "UChar8", 0) ]), keepAlive = keepAlive)
